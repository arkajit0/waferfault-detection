...
class Model_Finder:
    ...
    def get_best_params_for_svc(self,train_x,train_y):
        ...
        Revisions: None
                        ...
                        """
        self.logger_object.db_log(self.file_object, 'Entered the get_best_params_for_svm method of the Model_Finder class')
        try:
    ...
    def get_best_params_for_random_forest(self,train_x,train_y):
        """
                        ...
                        """
        self.logger_object.db_log(self.file_object, 'Entered the get_best_params_for_random_forest method of the Model_Finder class')
        try:
            # initializing with different combination of parameters
        ...
        try:
            ...
            try:
                ...
                if len(
                    ...
                    precision, recall, f1 = precision_score(test_y, self.prediction_knn), \
                                            recall_score(test_y, self.prediction_knn), f1_score(test_y,
                                                                                                self.prediction_knn)
                    ...
                    self.logger_object.db_log(self.file_object,
                                              {"Accuracy_score": str(self.knn_score), "model_name": "KNN",
            ...
            try:
                ...
                if len(
                    ...
                    model_analysis_dict = {"precision": precision, "recall": recall, "f1": f1, "conf": conf_matrix}
                    pickled_model = pickle.dumps(model_analysis_dict)
                    self.logger_object.db_log(self.file_object, {"Accuracy_score": str(self.random_forest_score),
                    ...
                    pickled_model = pickle.dumps(model_analysis_dict)
                    self.logger_object.db_log(self.file_object,
                                              {"AUC_score": str(self.random_forest_score), "model_name": "RandomForest",
                                               "cluster": str(self.cluster_no),
                                               "model_analysis": pickled_model})
            except:
            ...
            try:
                ...
                if len(
                    ...
                    pickled_model = pickle.dumps(model_analysis_dict)
                    self.logger_object.db_log(self.file_object,
                                              {"Accuracy_score": str(self.xgboost_score), "model_name": "XGBoost",
                                               "cluster": str(self.cluster_no),
                                               "model_analysis": pickled_model})
                else:
                    ...
                    pickled_model = pickle.dumps(model_analysis_dict)
                    self.logger_object.db_log(self.file_object,
                                              {"AUC_score": str(self.xgboost_score), "model_name": "XGBoost",
                                               "cluster": str(self.cluster_no),
                                               "model_analysis": pickled_model})
            except:
                self.xgboost = None
            ...
            try:
                ...
                self.prediction_decisiontree = self.decisiontree.predict(
                    test_x)  # Predictions using the Decision tree Model
                ...
                if len(
                    ...
                    precision, recall, f1 = precision_score(test_y, self.prediction_decisiontree), \
                                            recall_score(test_y, self.prediction_decisiontree), f1_score(test_y,
                                                                                                         self.prediction_decisiontree)
                    ...
                    self.decisiontree_score = roc_auc_score(test_y,
                                                            self.prediction_decisiontree)  # AUC for Decision tree
