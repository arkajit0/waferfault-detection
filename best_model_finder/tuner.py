    ...
    def get_best_model(self,train_x,train_y,test_x,test_y):
        ...
        try:
            ...
            model_name = [("XGBoost", self.xgboost, self.xgboost_score), ("RandomForest", self.random_forest, self.random_forest_score),
                          ...
                          ("KNN", self.knn, self.knn_score)]
            model_scores = [self.xgboost_score, self.random_forest_score, self.decisiontree_score, self.svc_score, self.knn_score]

            best_model = model_name[model_scores.index(max(model_scores))]

            model_name_store = ["XGBoost", "RandomForest", "DecisionTree", "SVC", "KNN"]
            model_scores_store = [self.xgboost_score*100, self.random_forest_score*100, self.decisiontree_score*100,
                                  self.svc_score*100, self.knn_score*100]
            self.logger_object.db_log(self.file_object,
                                      {"model_status": pd.DataFrame({"models": model_name_store, "scores": model_scores_store}),
